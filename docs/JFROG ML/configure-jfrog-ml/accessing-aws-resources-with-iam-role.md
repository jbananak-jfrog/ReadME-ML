...
Now that you created your IAM role you can use it to read data for CSV and Parquet [Data Sources](/docs/data-sources#batch-data-sources), access S3 files for [Build Configurations](/docs/advanced-build-concepts#build-configurations) or <Anchor label="Batch Executions" title="Automating Batch Execution" href="/docs/automating-batch-execution">Batch Executions</Anchor> just by referring the IAM Role ARN in the JFrog ML SDK or via the JFrog ML UI.
